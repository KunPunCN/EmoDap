...
def train(train_dataset, model, args, device):
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        p_macro_sim, r_macro_sim, f_macro_sim = evaluate(dev_dataset, model, args, args.device, args.dataset, args.dataset_path, args.topk)
...
def evaluate(dataset, model, args, device, task_name, data_path, topk):
    t0 = time.time()
    model.eval()
    sampler = SequentialSampler(dataset)
    dataloader = DataLoader(dataset, sampler=sampler, batch_size=args.evaluate_batch_size, collate_fn=custom_collate_fn)
    save_paths = os.path.join(data_path, str(task_name).lower()+f'_semantic_top{topk}_{t0}.txt')


    with open(save_paths, 'w', encoding='utf-8') as new_file_obj:
        with torch.no_grad():
            predict_labels_sim = []
            ...
            for step, batch in enumerate(dataloader):
                ...
                outputs = model(**inputs)
                max_sim_idx, candis = outputs
                # max_sim_idx = outputs

                x = ''
                for i in candis[0].tolist():
                    x = x + str(i) + ', '
                x = x.strip(', ')
                new_file_obj.write(x)
                new_file_obj.write('\n')
                ...
                # print(predict_labels_sim, true_labels)
    
            report = classification_report(true_labels, predict_labels_sim, output_dict=True)
            avg_p = report['weighted avg']['precision']
            avg_r = report['weighted avg']['recall']
            avg_fscore = report['weighted avg']['f1-score']

            cm = confusion_matrix(true_labels, predict_labels_sim)
            print(cm)
            x = np.sum(cm, axis=1)
...
if __name__=='__main__':
    ...
    parser.add_argument("--topk", type=int, default=8, help='Number of candis')
    ...
    # file_path
    parser.add_argument("--dataset_path", type=str, default='../data/ED/', help='where data stored')
    parser.add_argument("--run_seed", type=int, default=1, choices=[1,2,3,4,5])
    parser.add_argument("--dataset", type=str, default='ED', choices=['ED', 'GE', 'EI', 'EDOS'],
    ...
    args.dataset_path = os.path.join('../data', args.dataset, 'random_splits2', 'fold_{}'.format(args.run_seed))

    if not os.path.exists('checkpoints'):
        os.makedirs('checkpoints')
    args.checkpoint_dir = f'checkpoints/{args.dataset}_unseen_{str(args.run_seed)}.pth'
    ...
    model = EMMA(args.pretrained_model_name_or_path, add_auto_match, args.max_seq_len, args.k, args.topk)
    model.to(args.device)

    # train
    train_dataset = Dataset("train", args.dataset_path, args.unseen,
    ...
    p_macro_sim, r_macro_sim, f_macro_sim = evaluate(test_dataset, model, args, args.device, args.dataset, args.dataset_path, args.topk)
